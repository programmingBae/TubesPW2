...
@section('content')
    ...
    {{-- main content here --}}
    <form method="POST" action="{{route('penyaluranUangUpdate',  $penyaluran_uang->id)}}" enctype="multipart/form-data">
        @csrf
        <div style="margin-left: 10px; margin-top:15px" class="form-group">
            ...
            <select id="supplierType2" name="txtPenerima" class="form-control">
                @foreach($penerima_donasis as $k)
                    @if ($k->id == $penyaluran_uang->penerima_donasi_id)
            ...
            <input type="text" class="form-control" id="txtJumlah" name="txtJumlah" value="{{$penyaluran_uang->jumlah}}" >
